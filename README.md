...
|---|---|---|
| Node Exporter Full | NodeExporter.json | Host CPU, memory, disk, network |
| Docker Engine | DockerEngine.json | Docker daemon metrics |
| Docker Monitoring | DockerMonitoring.json | Container resource usage |
| cAdvisor Exporter | cAdvisor.json | Container-level metrics |
| PostgreSQL | PostgreSQL.json | Database performance |
| Loki Dashboard | Loki.json | Log throughput & errors |
| Prometheus | Prometheus.json | Prometheus internals |
| Nginx | Nginx.json | Nginx request metrics |
| Blackbox Exporter | BlackboxExporter.json | External probe results |
| Disk and Filesystem | DiskAndFilesystem.json | Disk usage & inodes |
| Grafana | Grafana.json | Grafana performance |
